...
router.get("/:uscid", restricted, (req, res) => {
	...
	if (!uscid) {
		res.status(404).json({ message: `The uscitem with the specified uscid ${uscid} does not exist.` });
	} else {
	...
	USCCitations.update(uscid, updatedUSCCodeItem)
		.then((uscitem) => {
			if (uscitem) {
				...
				res.status(404).json({ message: `Could not find uscitem with given id ${uscid}.` });
			}
		})
		.catch((err) => {
...
router.delete("/:uscid", restricted, (req, res) => {
	const uscid = req.params.uscid;
	if (!uscid) {
		res.status(404).json({ message: `The uscitem with the specified ID ${uscid} does not exist.` });
	}
