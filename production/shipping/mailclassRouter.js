	...
	MailClass.update(mcid, updatedMailClass)
		.then((mailclass) => {
			if (mailclass) {
				...
				res.status(404).json({ message: `Could not find mailclass with given id ${mcid}.` });
			}
		})
		.catch((err) => {
...
router.delete("/:mcid", restricted, (req, res) => {
	const mcid = req.params.mcid;
	if (!mcid) {
		res.status(404).json({ message: `The mailclass with the specified ID ${mcid} does not exist.` });
	}
